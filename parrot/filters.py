  ...
  def filter(self, para_phrases, fluency_threshold, device="cpu"):
    ...
    for para_phrase in para_phrases:
      input_ids = self.fluency_tokenizer(
          f"Sentence: {para_phrase}", return_tensors='pt', truncation=True)
...
class Diversity():
  ...
  def euclidean_ranker(self, input_phrase, para_phrases):
    ...
    for col in df.columns:
      tup = ([col], MinMaxScaler()) if col == "scores" else ([col], None)
      fields.append(tup) 

    mapper = DataFrameMapper(fields, df_out=True)
    return {
        row['paraphrase']: row['scores']
  ...
  def diff_ranker(self, input_phrase, para_phrases):
    ...
    for para_phrase in para_phrases:
      ...
      count = sum("+" in d or "-" in d for d in diff)
